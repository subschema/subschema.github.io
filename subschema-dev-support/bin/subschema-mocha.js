...
process.argv.push('--compilers',
    'js:' + path.resolve(__dirname, '..', 'babel-register'));
//process.argv.push('--require', path.resolve(__dirname, '..',
// 'babel-register'));
...
if (process.env.SUBSCHEMA_COVERAGE || process.env.SUBSCHEMA_COVERAGE_DIR
    || process.env.SUBSCHEMA_COVERAGE_USE_GLOBAL) {
    process.env.SUBSCHEMA_COVERAGE = 1;
    var coverage                   = process.env.SUBSCHEMA_COVERAGE_DIR
    ...
    process.argv.splice(2, 0, '--source-map=false', `--report-dir=${coverage}`,
